...
public:
    Raster()
    {
        static_assert(std::is_same<T, int>::value || std::is_same<T, short>::value || std::is_same<T, float>::value || std::is_same<T, double>::value, "Raster only accepts short, float or double.");
    }
