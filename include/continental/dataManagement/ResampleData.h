...
/// </summary>
class CONTINENTALDATAMANAGEMENT_EXPORT ResampleData
{
public:
    static int identifyColumn(const double x, const double xOrigin, const double cellSize);

    static int identifyRow(const double y, const double yOrigin, const double cellSize, const int rows);

    //Reajusta os atributos de um raster através de um raster de referência
    template<class T>
    static void adjustSpatialData(Raster<T> &rasterToAdjust, const Raster<T> &referenceRaster)
    {
    ...
    template<class T>
    static void excludeNoDataFromRaster(Raster<T> &rasterToAdjust, const Raster<T> &referenceRasterArea, const short switchValue, const double xYtolerance)
    {
