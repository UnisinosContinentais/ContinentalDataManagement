#ifndef CONTINENTALDATAMANAGEMENT_RASTER_H
#define CONTINENTALDATAMANAGEMENT_RASTER_H

#include <string>
#include <vector>
#include <stdexcept>
#include <limits>

namespace ContinentalDataManagement
{
	/// <summary>
	/// Classe de raster que deve ser obrigatoriamente herdada por classes derivadas
	/// </summary>
	/// <remarks></remarks>
    template <class T>
    class Raster
	{
        protected:
            // Numero Colunas
            size_t m_cols = 0;
            // Numero Linhas
            size_t m_rows = 0;
            double m_x_origin = 0;
            double m_y_origin = 0;
            // Resolução do raster
            double m_cellSize = 0;
            // Valor do NODATA
            short m_noDataValue = 0;

            std::vector<T> m_data;
            T m_maxValue;
        public:
            Raster() : m_maxValue(std::numeric_limits<T>::min())
            {
                static_assert(std::is_same<T, short>::value || std::is_same<T, float>::value, "Raster only accepts short or float.");
            }

            Raster(size_t rows, size_t cols, double xOrigin, double yOrigin, double cellSize, int noData) : Raster()
            {
                m_data.reserve(rows * cols);

                m_cols = cols;
                m_rows = rows;
                m_x_origin = xOrigin;
                m_y_origin = yOrigin;
                m_cellSize = cellSize;
                m_noDataValue = noData;
            }

            /// <summary>
            /// Retorna o número de linhas
            /// </summary>
            size_t getRows() const
            {
                return m_rows;
            }
            /// <summary>
            /// Retorna o número de colunas
            /// </summary>
            size_t getCols() const
            {
                return m_cols;
            }
            /// <summary>
            /// Retorna a resolução da grade
            /// </summary>
            double getCellSize() const
            {
                return m_cellSize;
            }
            /// <summary>
            /// Retorna o valor para o qual não se tem dados no raster
            /// </summary>
            short getNoDataValue() const
            {
                return m_noDataValue;
            }
            /// <summary>
            /// Retorna o número total de células
            /// </summary>
            size_t getTotalCells() const
            {
                return m_cols * m_rows;
            }
            /// <summary>
            /// Retorna a coordenada X do canto superior da grade
            /// </summary>
            double getXOrigin() const
            {
                return m_x_origin;
            }
            /// <summary>
            /// Retorna a coordenada Y do canto superior da grade
            /// </summary>
            double getYOrigin() const
            {
                return m_y_origin;
            }

            /// <summary>
            /// Faz uma copia completa dos dados de um raster
            /// </summary>
            Raster copyData()
            {

                Raster newRaster(this->getRows(), this->getCols(), this->getXOrigin(), this->getYOrigin(), this->getCellSize(), this->getNoDataValue());
                for (size_t i = 0; i < newRaster.getTotalCells(); ++i)
                    newRaster.setData(i, this->getData(i));

                return newRaster;
            }

            /// <summary>
            /// Retorna o máximo valor dos dados da matriz(y, x) do raster
            /// </summary>
            T getMaxValue() const
            {
                return m_maxValue;
            }

            T getData(const size_t position) const
            {
                assert(position < m_data.size());

                return m_data[position];
            }

            T getData(const size_t row, const size_t column) const
            {
                return getData(row * m_cols + column);
            }

            void setData(const size_t position, T value)
            {
                assert(position < m_data.size());

                if (value > m_maxValue)
                    m_maxValue = value;

                m_data[position] = value;
            }

            void setData(const size_t row, const size_t column, T value)
            {
                setData(row * m_cols + column, value);
            }

            std::vector<T> getCopyData() const
            {
                return m_data;
            }
	};
}

#endif // CONTINENTALDATAMANAGEMENT_RASTER_H
